...
public class AddBudgetCampaignCommandHandler : ChartDataRepository<string, decimal>, IProcessEvent<AddBudgetCampaignCommand>
{
    private const int MAX_ITEMS = 20;

    public AddBudgetCampaignCommandHandler(IMongoClient client) : base(StatisticsEnum.CampaignBudget, client)
    {
    }

    public async Task<bool> Process(AddBudgetCampaignCommand item)
    {
        try
        {
            if (Exists(item.ExternalId))
            {
                await UpdateChartData(item.ExternalId, old =>
                {
                    ...
                    {
                        old.datasets[0].data[old.datasets[0].data.Count - 1] +=Math.Round(item.Budget,2,MidpointRounding.AwayFromZero);
                    }
                    else
                    if (DateTime.Parse(old.labels.Last()).Date < DateTime.UtcNow.Date)
                    {
                        ...
                        old.datasets[0].data.Add(lastvalue+Math.Round(item.Budget,2,MidpointRounding.AwayFromZero));
                    }
                    return old;
                });
            }
            else
            {
                    ...
                    datasets = new List<Dataset<decimal>>{new Dataset<decimal>
                {
                  ...
                  borderWidth = 1,
                  data = new List<decimal>{Math.Round(item.Budget,2,MidpointRounding.AwayFromZero)},
                  label = "budget"
                }}
                });
            }
