...
public class AddProfitLinkCommandHandler : ChartDataRepository<string, decimal>, IProcessEvent<AddProfitLinkCommand>
{
    private const int MAX_ITEMS = 20;

    public AddProfitLinkCommandHandler(IMongoClient client) : base(StatisticsEnum.LinkProfit, client)
    {
    }

    public async Task<bool> Process(AddProfitLinkCommand item)
    {
        try
        {
            ...
            else
            {
                    ...
                    datasets = new List<Dataset<decimal>>{new Dataset<decimal>
                {
                  ...
                  borderWidth = 1,
                  data = new List<decimal>{Math.Round(item.Profit,2,MidpointRounding.AwayFromZero)},
                  label = "profit"
                }}
                });
            }
