...
public class HitWorker : BackgroundService
{
    ...
    private async Task ProcessHit(Hit item)
    {
        using (var dbtrans = _db.Database.BeginTransaction())
        {
            try
            {
                ...
                if (IPAddress.IsLoopback(item.Origin)) item.Origin = await NetworkUtil.GetRealLocalIP();

                var origin = item.Origin != null ? item.Origin.MapToIPv4().ToString() : "127.0.0.1";
